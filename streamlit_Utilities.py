...
SerpAPIWrapper.api_key=serp_api_key
...
def generate_Blog_SEO(topic):
    messages = [
    {"role": "system", "content": """You are trained to analyze a {topic} and generate (8) (SEO) keywords of a blog post depending upon the topic.Only generate (SEO) keywords.Response should be in the same language as {topic}"""},
...
def topic_generate(prompt):
    messages = [
    {"role": "system", "content": """Your role is to Analyze the {prompt} and generate (one) main (Topic) related to {prompt}. Get the idea from {prompt} and write (Topic).The (Topic) should be relevant to {prompt}.And (Topic) must be only (one).Response should contain only topic and focus on the main (topic) of {prompt}.Response contain at most 2 words."""},
